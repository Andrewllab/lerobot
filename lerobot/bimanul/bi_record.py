...
# action features example {gripper.pos: float, wrist_roll.pos: float, ...}
...
def bi_record_loop(
    ...
    **kwargs
    ...
    while timestamp < control_time_s:
        ...
        # add robot1 observation to the observation dict || align with the dataset observation features
...
@parser.wrap()
def bi_record(cfg: RecordConfig) -> LeRobotDataset:
    init_logging()
    ...
    robot1 = make_robot_from_config(cfg.robot1) if cfg.robot1 is not None else None
    ...
    num_arms = [robot, robot1, teleop, teleop1]
    for arm in num_arms:
        if arm is None:
            raise ValueError(f"{arm} is missing.")

    # robot.action_features example {"gripper.pos": float, "wrist_roll.pos": float, ...}
    action_features = hw_to_dataset_features(robot.action_features, "action", cfg.dataset.video)

    # robot.observation_features example {"gripper.pos": float, "wrist_roll.pos": float, ...,
    #                               "camera_name1": (cam1_h, cam1_w, 3), "camera_name2": (cam2_h, cam2_w, 3), ...}
    ...
    if robot1:
        ...
        action_features['action']['shape'] = (12,) # robots joints

        obs_features['observation.state']['names'] += robo1_names
        obs_features['observation.state']['shape'] = (12,) # robots joints
    ...
    if cfg.resume:
        ...
        # cameras should be set to robot, not robot1
    ...
    for recorded_episodes in range(cfg.dataset.num_episodes):
        ...
        if not events["stop_recording"] and (
            ...
            # don't set the dataset, so that the observation and action frame are not added into the dataset
            bi_record_loop(
                robot=robot,
                robot1=robot1,
                events=events,
